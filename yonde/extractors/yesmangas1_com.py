...
class YesMangas(MainExtractor):
    ...
    def get_capitulos(self):
        self._manga_name = self.re_findall('manga/([\w-]+)-ym', self._url)[0]
        numero_cap_pattern = self.re_compile('ym\d+/([\d.-]+)')
        capitulo_pattern = self.re_compile('(ym\d+/[\w.-]+)')
        if self.re_findall(capitulo_pattern, self._url):
            ...
            if not self._cap_inicial and not self._cap_final:
                url_capitulos = [i.get('href') for i in self._soup.xpath('//*[@id="capitulos"]/div/a')]
                url_capitulos.reverse()
                [self._capitulos.append(i) for i in [self.re_findall(capitulo_pattern, a)[0] for a in url_capitulos]]
            else:
                url_capitulos = [i.get('href') for i in self._soup.xpath('//*[@id="capitulos"]/div/a') if
    ...
    def get_url_imagens(self):
        return [i.get('src') for i in self._soup.cssselect('div.read-slideshow a img')]
    ...
    def runner(self):
        ...
        for i in self._capitulos:
            ...
            while True:
                try:
                    req = self._session.get(f'https://yesmangas1.com/manga/{self._manga_name}-{i}', random_ua=True).text
